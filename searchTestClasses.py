...
#########################################################
# Test classes for Questions 1-5                        #
#########################################################
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents, astar_search):
        alg = getattr(search, self.alg)
        ...
        heuristic = getattr(astar_search, self.heuristicName) if self.heuristicName != None else None
    ...
    def execute(self, grades, moduleDict, solutionDict):
        search = moduleDict['search']
        searchAgents = moduleDict['searchAgents']
        astar_search = moduleDict['astar_search']
        gold_solution = [str.split(solutionDict['solution']), str.split(solutionDict['rev_solution'])]
        gold_expanded = max(int(solutionDict['expanded_nodes']), int(solutionDict['rev_expanded_nodes']))

        solution, expanded, error = self.getSolInfo(search, searchAgents, astar_search)
        if error != None:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('%s' % error)
            return False
...
class HeuristicTest(testClasses.TestCase):
    ...
    def setupProblem(self, searchAgents, astar_search):
        ...
        state = problem.getStartState()
        heuristic = getattr(astar_search, self.heuristicName)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem, state, heuristic = self.setupProblem(searchAgents, astar_search)
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        problem, _, heuristic = self.setupProblem(searchAgents, astar_search)
